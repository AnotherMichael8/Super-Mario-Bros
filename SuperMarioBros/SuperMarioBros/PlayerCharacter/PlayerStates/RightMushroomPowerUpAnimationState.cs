...
{
    public class RightMushroomPowerUpAnimationState : AbstractPlayerState, IRightFacing
    {
        private AbstractPlayerState previousState;
        private IPlayerSprite previousSprite;
        private int frameCnt;
        public RightMushroomPowerUpAnimationState(Player player, AbstractPlayerState state) : base(player)
        {
            previousSprite = player.Sprite;
            player.Sprite = PlayerSpriteFactory.Instance.CreateRightAnimationMushroomSprite();
            //Speed = 0;
            //JumpingSpeed = 0;
            if (JumpingSpeed > 64)
                JumpingSpeed = 64;
