...
namespace SuperMarioBros.PlayerCharacter.PlayerStates
{
    public class LeftMushroomPowerUpAnimationState : AbstractPlayerState, ILeftFacing
    {
        ...
        public LeftMushroomPowerUpAnimationState(Player player, AbstractPlayerState state) : base(player)
        {
            previousSprite = player.Sprite;
            player.Sprite = PlayerSpriteFactory.Instance.CreateLeftAnimationMushroomSprite();
            //Speed = 0;
            //JumpingSpeed = 0;
            if (JumpingSpeed > 64)
                JumpingSpeed = 64;
