...
{
    public abstract class AbstractPlayerSprite : IPlayerSprite
    {
        ...
        public AbstractPlayerSprite(Texture2D texture, AbstractPlayerState.PowerUps powerUp = AbstractPlayerState.PowerUps.NONE)
        {
            this.texture = texture;
            frameCounter = 0;
            switch(powerUp)
            {
                case (AbstractPlayerState.PowerUps.MUSHROOM):
                    //Distance in pixels from small mario to big mario in spritesheet
                    updatePowerUpSprite = 24;
                    heightMultiplier = 2;
                    break;
                case (AbstractPlayerState.PowerUps.FIREFLOWER):
