...
namespace SuperMarioBros.Blocks.BlockSprites
{
    public class HorizontalPipeSprite : IPipeSprite
    {
        ...
        private readonly Rectangle HorizontalBottomLeftPipe = new Rectangle(103, 230, 14, 16);
        private readonly Rectangle HorizontalBottomRightPipe = new Rectangle(103, 247, 14, 16);
        ...
        public void Draw(SpriteBatch spriteBatch, Vector2 position, Color color, int pipeHeight, ICollision enterbleSide)
        {
            ...
            if (inFrame)
            {
                ...
                if (onLeft)
                {
                    ...
                    spriteBatch.Draw(texture, destinationRectangle, HorizontalTopRightPipe, color, 0, new Vector2(0), spriteEffect, .01f);
                    destinationRectangle.X += (int)Globals.BlockSize;
                    destinationRectangle.Y -= (int)Globals.BlockSize;
                }
                for (int i = 0; i < pipeHeight - 1; i++)
                {
                    spriteBatch.Draw(texture, destinationRectangle, HorizontalBottomLeftPipe, color, 0, new Vector2(0), spriteEffect, .01f);
                    destinationRectangle.Y += (int)Globals.BlockSize;
                    spriteBatch.Draw(texture, destinationRectangle, HorizontalBottomRightPipe, color, 0, new Vector2(0), spriteEffect, .01f);
                    destinationRectangle.X += (int)Globals.BlockSize;
                    destinationRectangle.Y -= (int)Globals.BlockSize;
                }
                if (!onLeft)
